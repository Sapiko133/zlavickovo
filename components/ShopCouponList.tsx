...
export function ShopCouponRow({ coupon, capitalized }: { coupon: any; capitalized: string }) {
...
interface ShopCouponListProps {
  capitalized: string;
  coupons: any[];
  /** Typ zoznamu — určuje text prázdneho stavu. */
  kind: "kupony" | "akcie";
  shopUrl?: string;
}

/**
 * Zoznam kupónov ALEBO akcií pre jednu sekciu stránky obchodu.
 * Kupóny a akcie sa držia oddelene — každý zoznam je vlastná sekcia,
 * volajúci filtruje (kód / bez kódu) ešte pred odovzdaním.
 */
export default function ShopCouponList({ capitalized, coupons, kind, shopUrl }: ShopCouponListProps) {
  return (
      ...
      {coupons.length > 0 ? (
        <div>
          {coupons.map((coupon: any, i: number) => (
            <ShopCouponRow key={coupon.id || i} coupon={coupon} capitalized={capitalized} />
          ))}
        </div>
      ) : (
        <div style={{
          ...
          <div style={{ fontSize: 40, marginBottom: 12 }}>{kind === "kupony" ? "🏷️" : "🔥"}</div>
          <div style={{ fontSize: 15, fontWeight: 600, color: T.textSecond, marginBottom: 6 }}>
            Momentálne žiadne {kind === "kupony" ? "kupóny s kódom" : "akcie"} pre {capitalized}
          </div>
          <div style={{ fontSize: 13, color: T.textMuted }}>
            {kind === "kupony"
              ? "Pozri AI sekciu nižšie — hľadáme kódy za teba."
              : "Skús kupóny s kódom vyššie alebo prejdi priamo do obchodu."}
          </div>
          {shopUrl && (
            <a
              href={shopUrl}
