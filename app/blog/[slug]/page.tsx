...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Inter', system-ui, sans-serif", color: "var(--text)" }}>
      ...
      {/* Hero obrázok článku */}
      <div style={{ position: "relative", height: "clamp(200px, 30vw, 420px)", overflow: "hidden", background: "var(--step-bg)" }}>
        <img
          src={imgUrl}
          alt={post.title}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px 80px", display: "flex", gap: 48, alignItems: "flex-start" }}>
        ...
        <article style={{ flex: 1, minWidth: 0, maxWidth: 720 }}>
          <div style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
              {post.shop && <span style={{ fontSize: 12, color: "var(--text2)" }}>{post.shop}</span>}
              <span style={{ fontSize: 12, color: "var(--text2)" }}>{new Date(post.date).toLocaleDateString("sk-SK")}</span>
            </div>
            <h1 style={{ fontSize: "clamp(24px, 4vw, 36px)", fontWeight: 800, letterSpacing: "-0.5px", lineHeight: 1.2, margin: "0 0 16px", color: "var(--text)" }}>{post.title}</h1>
            <p style={{ fontSize: 16, color: "var(--text2)", lineHeight: 1.7, margin: 0 }}>{post.description}</p>
          </div>

          <hr style={{ border: "none", borderTop: "1px solid var(--border)", margin: "32px 0" }} />

          <style>{`
            article h2 { font-size: 22px; font-weight: 700; margin: 36px 0 14px; letter-spacing: -0.3px; color: var(--text); }
            article h3 { font-size: 17px; font-weight: 700; margin: 24px 0 10px; color: var(--text); }
            article p  { font-size: 15px; line-height: 1.8; color: var(--text2); margin: 0 0 16px; }
            article ul { padding-left: 22px; margin: 0 0 16px; }
            article li { font-size: 15px; line-height: 1.8; color: var(--text2); margin-bottom: 6px; }
          ...
          {shopSlug && (
            <div style={{ marginTop: 48, padding: "28px", background: "linear-gradient(135deg, #f5f3ff, #eff6ff)", borderRadius: 16, border: "1px solid rgba(124,58,237,0.15)", textAlign: "center" }}>
        ...
        <aside style={{ width: 280, flexShrink: 0, position: "sticky", top: 72, display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ background: "var(--card)", borderRadius: 16, padding: "20px", border: "1px solid var(--border)" }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 16, color: "var(--text)" }}>Ďalšie články</div>
            {getAllPosts().filter(p => p.slug !== post.slug).slice(0, 5).map(p => (
              <a key={p.slug} href={`/blog/${p.slug}`} style={{ display: "block", padding: "10px 0", borderBottom: "1px solid var(--border)", textDecoration: "none" }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text)", lineHeight: 1.4 }}>{p.title}</div>
                <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 3 }}>{categoryLabel(p.category)}</div>
              </a>
            ))}
          </div>
          {shopSlug && (
